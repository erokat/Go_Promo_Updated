...
BEGIN
    ...
    -- 4. Проверка фискального кода (12 цифр, начинается на 0000)
    IF length(NEW.receipt) <> 12 OR NEW.receipt !~ '^\d{12}$' THEN
        RAISE EXCEPTION 'Неправильный код чека. Код должен состоять из 12 цифр';
    END IF;
    
    IF NOT NEW.receipt LIKE '0000%' THEN
        RAISE EXCEPTION 'Неправильный код чека. Код должен начинаться на 0000';
    END IF;
...
-- Автоматическое добавление новых пользователей из Auth в таблицу администраторов (admin_users)
